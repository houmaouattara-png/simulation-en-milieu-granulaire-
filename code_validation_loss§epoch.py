...
def plot_force_chains_comparison(
    ...
    for ax, values, threshold, vmax, title in zip(
        ...
        # Grains colorés selon l'importance
...
# Listes optionnelles si tu veux garder aussi les MAE
train_maes = []
val_maes = []
...
best_model_path = os.path.join(output_dir, "best_edge_force_gnn.pth")
...
train_loss_path = os.path.join(output_dir, "train_loss_curve.png")
...
plt.title("Loss de validation")
plt.grid(True)
plt.tight_layout()

val_loss_path = os.path.join(output_dir, "val_loss_curve.png")
...
sample_norm = val_graphs[sample_idx].to(device)
sample_raw = raw_val_graphs[sample_idx]
