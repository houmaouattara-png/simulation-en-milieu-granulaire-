...
def build_graph(nodes_path, forces_path):
    ...
    # Conversion des colonnes des noeuds
    ...
    # Conversion des colonnes des forces
...
# CHARGEMENT DE PLUSIEURS GRAPHES
...
# PROGRAMME PRINCIPAL : TRAIN et  VALIDATION
...
# Normalisation sur entrainement
...
raw_val_graphs, val_ids = load_graphs(base_dir, 500, 999)

print("\nRÉSUMÉ CHARGEMENT VALIDATION")
print("Nombre de graphes validation :", len(raw_val_graphs))
print("IDs validation :", val_ids)

if len(raw_val_graphs) == 0:
    raise ValueError("Aucun graphe de validation chargé.")

val_graphs = copy.deepcopy(raw_val_graphs)

#  normalisation sur validation 
val_graphs = apply_normalization(val_graphs, x_mean, x_std, y_mean, y_std)
...
torch.save(model.state_dict(), os.path.join(output_dir, "best_edge_force_gnn.pth"))
print("\nModèle sauvegardé : best_edge_force_gnn.pth")


#  TEST VISUEL SUR UN GRAPHE DE VALIDATION
...
if target_id in val_ids:
    sample_idx = val_ids.index(target_id)
else:
    raise ValueError(f"Le fichier de validation {target_id} n'a pas été chargé.")

sample_norm = val_graphs[sample_idx].to(device)   # pour la prédiction
sample_raw = raw_val_graphs[sample_idx]           # pour le tracé
...
with torch.no_grad():
    ...
    print("\nRESULTATS SUR UN GRAPHE DE VALIDATION")
    print(f"Fichier validation : {target_id}")
    print(f"MSE normalisé : {mse:.6f}")
    print(f"MAE normalisé : {mae:.6f}")

    # Dénormalisation des forces
...
# Tracé DEM vs GNN sur un graphe de validation
...
plot_force_chains_comparison(
    ...
    true_real=true_real,
    title_true="Chaînes de force vraies (DEM)",
    title_pred="Chaînes de force prédites (GNN) - Validation",
    force_threshold_ratio=1e-12,
    save_path=os.path.join(output_dir, f"comparaison_dem_gnn_val_{target_id}.png")
)

print(f"\nImage sauvegardée : comparaison_dem_gnn_val_{target_id}.png")
